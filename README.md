...
* **Parameters:**
  ...
  * `priceInfo:PriceInfo` The data price symbol and price, now only supports AO’s test token AOCRED(TestToken), minimum unit to use AOCRED(1 means 0.001 AOCRED).
...
Submit a task to PADO Network. And must pay the data fee corresponding to the dataId and the computing fee of the PADO Node. Now each task charges 0.001 AOCRED(TestToken) per computing node (the minimum unit corresponding to AOCRED is 1)
