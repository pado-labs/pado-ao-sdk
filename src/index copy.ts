...
export const submitData = async (
  ...
  if (extParam && extParam.uploadParam && StorageType.ARSEEDING === extParam.uploadParam.storageType) {
    ...
    transactionId = await submitDataToArseeding(arweave, encryptedData.enc_msg, wallet, extParam.uploadParam.symbolTag);
  } else {
 ...
 * @param wallet - The ar wallet json object, this wallet must have wAR. Pass `window.arweaveWallet` in a browser.
...
export const submitTask = async (dataId: string, dataUserPk: string, wallet: any): Promise<string> => {
  ...
  //get node price
  const nodePrice = await fetchComputationPrice(symbol);
  ...
  try {
    if (symbol === 'AOCRED') {
      ...
      await transferWARToTask(totalPrice.toString(), signer);
    }
  } catch (err) {
